...
# Create a pipeline with ColumnTransformer for preprocessing and Support Vector Machine (SVM)
...
pipeline = Pipeline([
    ...
    ('classifier', SVC(random_state=42))  # Using Support Vector Machine
])
...
def recommendation(N, P, k, temperature, humidity, ph, rainfall):
    ...
    prediction_prob = pipeline.decision_function(features)  # Using decision_function for SVM
    predicted_class = pipeline.predict(features)
    
    # Extracting the confidence level for the predicted class
    # Using expit to scale the decision function output
    confidence = np.max(expit(prediction_prob)) 
